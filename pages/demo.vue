<template>
  <main
    class="v-demo"
  >
    <div
      class="v-demo__content app-show-background-on-nav"
    >
<!--      <div class="v-demo__section">-->
<!--        <app-citation-->
<!--          text="Lorem Que fait Modus ?"-->
<!--        />-->
<!--      </div>-->

      <div class="v-demo__section">
        <app-text-content
          text="
                        <p>La Fondation Modus participe à la décarbonation des mobilités à Genève.</p>
                        <p>Elle est un accélérateur du changement en soutenant et en mettant en réseau les acteurs de la transition des mobilités, en expérimentant des solutions concrètes, en impulsant des idées originales, en créant de nouvelles connaissances et en développant des services innovants.</p>
                        <p>La Fondation Modus n’est pas partisane, elle agit comme un laboratoire consacré à la transition des mobilités à Genève et dans sa région.</p>
                    "
          variant="yellow-block"
        />
      </div>

      <div class="v-demo__section v-demo__section--no-margin">
        <app-project-item
          img-src="/temp/project_item_cover.png"
          project-type="bibliotheque"
          title="lorem title"
          content="
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium asperiores consequatur cum doloremque ex facilis iusto molestias placeat qui quia quibusdam reiciendis, voluptatem! Architecto eligendi ipsa nobis quasi tempora voluptas!</p>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores culpa cum eligendi enim error et eveniet facere in modi molestias nisi nostrum, numquam placeat ratione saepe totam unde vitae voluptatum!</p>
                    "
        />
      </div>


      <div class="v-demo__section v-demo__section--no-margin">
        <app-actuality
          src="images/8.jpg"
          title="Dernier projet"
          description="
                        <p>lorem</p>
                    "
        />
      </div>









      <div
        style="width: 100%; overflow: hidden"
      >
        <section
          class="v-demo__section v-demo__section--intro"
        >
          <img class="v-demo__section__graphic-items v-demo__section__graphic-items--m" alt="graphic image"
               src="../assets/items/m.svg"/>
          <img class="v-demo__section__graphic-items v-demo__section__graphic-items--o" alt="graphic image"
               src="../assets/items/o.svg"/>
          <img class="v-demo__section__graphic-items v-demo__section__graphic-items--du" alt="graphic image"
               src="../assets/items/du.svg"/>
          <img class="v-demo__section__graphic-items v-demo__section__graphic-items--s" alt="graphic image"
               src="../assets/items/s.svg"/>
          <div
            class="v-demo__grid__coll-full"
          >
            <div
              class="v-demo__section__title"
            >
              <h2>Que fait Modus ?</h2>
            </div>
            <p>
              La Fondation Modus participe à la décarbonation des mobilités à Genève. Elle est un accélérateur du changement en soutenant et en mettant en réseau les acteurs de la transition des mobilités, en expérimentant des solutions concrètes, en impulsant des idées originales, en créant de nouvelles connaissances et en développant des services innovants. La Fondation Modus n’est pas partisane, elle agit comme un laboratoire consacré à la transition des mobilités à Genève et dans sa région.
            </p>
          </div>
        </section>
      </div>

      <section
        class="v-demo__section"
      >
        <div
          class="v-demo__section__title"
        >
          <h2>Ses objectifs</h2>
        </div>

        <div
          class="v-demo__section__content v-demo__section__content--sticky v-demo__grid v-demo__grid--with-gutter"
        >
          <div
            class="v-demo__grid__coll-1-2"
          >
            <div
              class="v-demo__section__content--sticky__coll__text-sticky"
            >
              <ul>
                <li>Soutenir les acteurs de la mobilité dans leurs projets</li>
                <li>Stimuler les rencontres entre les&nbsp;acteurs</li>
                <li>Fournir des approches, des idées et des connaissances originales</li>
                <li>Proposer de nouveaux imaginaires de la&nbsp;mobilité</li>
                <li>Documenter et disséminer le&nbsp;changement</li>
              </ul>
            </div>
          </div>
          <div
            class="v-demo__grid__coll-1-2 v-demo__section__content--sticky__coll"
          >
            <div
              class="v-demo__section__content--sticky__coll__element-sticky"
            >
              <img
                src="/images/compress/116.jpg"
                alt="image de ville abstraite"
              />
            </div>
          </div>
        </div>

      </section>


      <section
        class="v-demo__section v-demo__section--no-padding"
      >
        <div
          class="v-demo__section__title"
        >
          <h3>Présentation de la Fondation&nbsp;Modus</h3>
        </div>
        <div
          class="v-demo__grid__coll-full"
        >
          <div
            class="v-demo__section__video-box"
          >
            <div
              class="v-demo__section__video-box__player v-demo__section__video-box__player--xl"
            >
              <app-video vimeo-id="886956196"/>
            </div>
          </div>
        </div>
      </section>

      <section
        class="v-demo__section"
        style="margin-top: 2rem"
      >
        <div
          class="v-demo__section__title"
        >
          <h2>Les dispositifs de Modus</h2>
        </div>
        <div
          class="v-demo__section__content v-demo__grid v-demo__grid--wrap v-demo__grid--with-gutter"
        >
          <img style="max-width: 25rem; margin: 2rem auto;" alt="icon" src="../assets/shema_new/intro.svg"/>
        </div>

        <div
          class="v-demo__section__title"
        >
          <h3>Présentation des dispositifs</h3>
        </div>

        <div
          class="v-demo__section__content v-demo__section__content--list-with-img"
        >
          <div class="v-demo__section__content--list-with-img__item">
            <img class="v-demo__section__content--list-with-img__item__img"
                 alt="illustre une liste"
                 src="../assets/shemat_3/01.svg"
            />
            <div class="v-demo__section__content--list-with-img__item__text"
            >
              <div class="v-demo__section__content--list-with-img__item__text__title"
              >La plateforme:
                <br>servir et réunir les acteurs de la mobilité
              </div>
              <div class="v-demo__section__content--list-with-img__item__text__content"
              >La plateforme anime l’écosystème de la mobilité en organisant des événements, en mettant en
                relation les secteurs privés, publics, académiques et associatifs tout en leur
                fournissant des outils et des moyens.
              </div>
            </div>
          </div>
          <div class="v-demo__section__content--list-with-img__item">
            <img class="v-demo__section__content--list-with-img__item__img"
                 alt="illustre une liste"
                 src="../assets/shemat_3/02.svg"
            />
            <div class="v-demo__section__content--list-with-img__item__text"
            >
              <div class="v-demo__section__content--list-with-img__item__text__title"
              >Le laboratoire:
                <br>innover et partager pour améliorer la mobilité
              </div>
              <div class="v-demo__section__content--list-with-img__item__text__content"
              >Le laboratoire expérimente de nouvelles approches pour transformer les comportements de mobilité en améliorant la compréhension des besoins et en expérimentant des solutions innovantes. Celles-ci sont alors mises à disposition des pouvoirs publics.
              </div>
            </div>
          </div>
          <div class="v-demo__section__content--list-with-img__item">
            <img class="v-demo__section__content--list-with-img__item__img"
                 alt="illustre une liste"
                 src="../assets/shemat_3/03.svg"
            />
            <div class="v-demo__section__content--list-with-img__item__text"
            >
              <div class="v-demo__section__content--list-with-img__item__text__title"
              >La bibliothèque:
                <br>réunir et conserver les connaissances sur la mobilité
              </div>
              <div class="v-demo__section__content--list-with-img__item__text__content"
              >La bibliothèque regroupe et produit des informations ouvertes sur la mobilité à Genève pour les mettre à disposition de ses acteurs.
              </div>
            </div>
          </div>
          <div class="v-demo__section__content--list-with-img__item">
            <img class="v-demo__section__content--list-with-img__item__img"
                 alt="illustre une liste"
                 src="../assets/shemat_3/04.svg"
            />
            <div class="v-demo__section__content--list-with-img__item__text"
            >
              <div class="v-demo__section__content--list-with-img__item__text__title"
              >La Fabrique des imaginaires:
                <br>communiquer et changer les imaginaires de la mobilité
              </div>
              <div class="v-demo__section__content--list-with-img__item__text__content"
              >La fabrique des imaginaires cherche à transformer les comportements en changeant les images et les valeurs sociales liées à la mobilité. Grâce à une large communication et à la production de contenu, elle veut toucher les experts et le grand public.
              </div>
            </div>
          </div>
          <div class="v-demo__section__content--list-with-img__item">
            <img class="v-demo__section__content--list-with-img__item__img"
                 alt="illustre une liste"
                 src="../assets/shemat_3/05.svg"
            />
            <div class="v-demo__section__content--list-with-img__item__text"
            >
              <div class="v-demo__section__content--list-with-img__item__text__title"
              >La Plantation:
                <br>financer et incuber les entreprises innovantes de mobilité
              </div>
              <div class="v-demo__section__content--list-with-img__item__text__content"
              >La plantation finance de jeunes entreprises et soutient leur développement, favorisant ainsi la transition des mobilités.
              </div>
            </div>
          </div>
        </div>
      </section>

      <section class="v-demo__section"
      >
        <div
          class="v-demo__section__title"
        >
          <h2>Soumettre un projet</h2>
        </div>

        <div
          class="v-demo__section__content v-demo__grid v-demo__grid--with-gutter"
        >
          <app-form/>
        </div>

      </section>


      <section
        class="v-demo__section"
      >
        <div
          class="v-demo__section__title"
        >
          <h2>Pourquoi Modus ?</h2>
        </div>

        <div
          class="v-demo__section__content v-demo__grid v-demo__grid--with-gutter"
        >
          <p>Le changement climatique est le défi du 21e siècle. En Suisse, les transports représentent 31% des émissions de gaz à effet de serre contre 24% dans le monde. La mobilité constitue donc 30% du problème, mais également 30% de la solution. La Fondation Modus relève ce défi en soutenant les acteurs de la décarbonation à Genève pour tomber à 0% de carbone dans la mobilité vers 2050.</p>
        </div>

      </section>

      <section
        class="v-demo__section"
      >
        <div
          class="v-demo__section__title"
        >
          <h3 style="margin-top: 0; margin-bottom: 2rem">Notre diagnostic</h3>
        </div>
        <div
          class="v-demo__section__content v-demo__grid v-demo__grid--wrap v-demo__grid--with-gutter app-schema_1 app-schema_animation"
        >
          <img class="app-schema_1_7" alt="icon" src="../assets/schema_1/1-7.svg"/>
          <img class="app-schema_1_8" alt="icon" src="../assets/schema_1/1-8.svg"/>
          <img class="app-schema_1_9" alt="icon" src="../assets/schema_1/1-9.svg"/>
          <img class="app-schema_1_3" alt="icon" src="../assets/schema_1/1-3.svg"/>
          <img class="app-schema_1_4" alt="icon" src="../assets/schema_1/1-4.svg"/>
          <img class="app-schema_1_5" alt="icon" src="../assets/schema_1/1-5.svg"/>
          <img class="app-schema_1_6" alt="icon" src="../assets/schema_1/1-6.svg"/>
          <img class="app-schema_1_10 is-circle" alt="icon" src="../assets/schema_1/1-10.svg"/>
          <img class="app-schema_1_1 is-circle" alt="icon" src="../assets/schema_1/1-1.svg"/>
          <img class="app-schema_1_2 is-circle" alt="icon" src="../assets/schema_1/1-2.svg"/>
          <img class="app-schema_1_0 not-animated" alt="icon" src="../assets/schema_1/1-0.svg"/>
          <img class="app-schema_1_11 not-animated" alt="icon" src="../assets/schema_1/1-11.svg"/>
        </div>

        <div
          class="v-demo__section__title"
        >
          <h3 style="margin-top: 5rem">Nos compétences</h3>
        </div>
        <div
          class="v-demo__section__content v-demo__grid v-demo__grid--wrap v-demo__grid--with-gutter app-schema_2 app-schema_animation"
        >
          <img class="app-schema_2_0" alt="schema" src="../assets/schema_2/2_0.svg"/>
          <img class="app-schema_2_1" alt="schema" src="../assets/schema_2/2-1.svg"/>
          <img class="app-schema_2_2" alt="schema" src="../assets/schema_2/2-2.svg"/>
          <img class="app-schema_2_3" alt="schema" src="../assets/schema_2/2-3.svg"/>
          <img class="app-schema_2_4" alt="schema" src="../assets/schema_2/2-4.svg"/>
          <img class="app-schema_2_6 is-circle" alt="schema" src="../assets/schema_2/2-6.svg"/>
          <img class="app-schema_2_5 not-animated" alt="schema" src="../assets/schema_2/2-5.svg"/>
        </div>
      </section>


      <section
        class="v-demo__section"
      >
        <div
          class="v-demo__section__title"
        >
          <h2>Le collectif</h2>
        </div>



        <div
          class="v-demo__section__content v-demo__grid v-demo__grid--wrap v-demo__grid--with-gutter"
        >
          <div
            style="width: 100%"
            class="v-demo__section__title"
          >
            <h3 style="margin-bottom: 1rem">Le Hub</h3>
          </div>
          <div
            class="v-demo__grid__coll-full"
          >
            <div class="v-demo__section__content v-demo__grid v-demo__grid--wrap v-demo__grid--with-gutter">
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/01©RaphaelleMueller.jpeg"
                  name="Dr. Guillaume Drevon"
                  desc="Directeur de la Fondation"
                  :is-variante="false"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/MODUS-Portrait_SarahClozel©RaphaelleMueller-1.jpg"
                  name="Sarah Clozel"
                  desc="Cheffe de projet"
                  :is-variante="false"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/MODUS-Portrait_DamienCataldi©RaphaelleMueller-1.jpg"
                  name="Damien Cataldi"
                  desc="Chef de projet"
                  :is-variante="false"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/MODUS-Portrait_MatthieuGautrot©RaphaelleMueller-1.jpg"
                  name="Matthieu Gautrot"
                  desc="Chef de projet"
                  :is-variante="false"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/MODUS-Portrait_GabrielaZwicky©RaphaelleMueller-1.jpg"
                  name="Gabriela Zwicky"
                  desc="Responsable administrative"
                  :is-variante="false"
                />
              </div>
            </div>
          </div>


          <div
            style="width: 100%"
            class="v-demo__section__title"
          >
            <h3 style="margin-bottom: 1rem">Conseil scientifique et conseil de fondation</h3>
          </div>
          <div
            class="v-demo__grid__coll-full"
          >
            <div class="v-demo__section__content v-demo__grid v-demo__grid--wrap v-demo__grid--with-gutter">
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/11©RaphaelleMueller.jpeg"
                  name="Prof. Patrick Saudan"
                  desc="Président du Conseil de Fondation"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/12©RaphaelleMueller.jpeg"
                  name="Yves Bersier"
                  desc="Membre du Conseil de Fondation"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/02©RaphaelleMueller.jpeg"
                  name="Prof. Yves Delacrétaz"
                  desc="Membre du Conseil de Fondation et membre du Conseil Scientifique"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/03©RaphaelleMueller.jpeg"
                  name="Prof. Vincent Kaufmann"
                  desc="Vice-président du Conseil de Fondation et membre du Conseil Scientifique"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/04©RaphaelleMueller.jpeg"
                  name="Jenny Leuba"
                  desc="Membre du Conseil Scientifique"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/09©RaphaelleMueller.jpeg"
                  name="Marc Maugué"
                  desc="Membre du Conseil de Fondation"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/05©RaphaelleMueller.jpeg"
                  name="Dre. Lisa Moussaoui"
                  desc="Membre du Conseil Scientifique"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  name="Dr. Emmanuel Ravalet"
                  desc="Membre du Conseil Scientifique"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/10©RaphaelleMueller.jpeg"
                  name="Thierry Renaud"
                  desc="Membre du Conseil de Fondation"
                />
              </div>
              <div
                class="v-demo__grid__coll-1-4"
              >
                <app-image
                  src="photos-profiles/06©RaphaelleMueller.jpeg"
                  name="Dr. Jean Simos"
                  desc="Membre du Conseil Scientifique"
                />
              </div>

            </div>
          </div>















        </div>

      </section>
    </div>



  </main>
</template>


<script lang="ts" setup>

import AppVideo from "~/components/AppVideo.vue";
import {arrayOfH2TitleIdInCurrentPage, cookieIsValidate} from "~/composable/main";
import {getValueToPercent} from "~/utils/precent";


import {bodyScrollInfo} from "~/composable/main";

useHead({
  title: 'modus',
  meta: [
    {
      name: 'description',
      content: 'modus. pour une mobilité durable à Genève',
    }
  ],
})

const h2TitleIdInCurrentPage = arrayOfH2TitleIdInCurrentPage()

onMounted(() => {
  nextTick(() => {
    h2TitleIdInCurrentPage.value =
      Array.from(document.querySelectorAll('.v-demo__section__title h2'))
        .map((htmlHeadingElement) => {


          if (!htmlHeadingElement.id)
            htmlHeadingElement.id = encodeURIComponent(htmlHeadingElement.textContent || 'generateID')

          return {
            anchor: htmlHeadingElement.id,
            name: (htmlHeadingElement as HTMLElement).innerText,
          }
        })

    interactionWithElementToAnimated()
  })
})

function interactionWithElementToAnimated() {
  const elementsToSetBackgroundNav = document.querySelectorAll('.app-schema_animation')

  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.classList.add('is-active')
      } else {
        entry.target.classList.remove('is-active')
      }
    })
  }, {
    rootMargin: `0px 0px ${window.innerHeight / 5 * -1}px 0px`
  })

  elementsToSetBackgroundNav.forEach((value) => {
    observer.observe(value)
  })
}
</script>


<style lang="scss" scoped>
.v-demo__section {
  box-sizing: border-box;
  padding-left: var(--app-gutter);
  padding-right: var(--app-gutter);
  max-width: 1300px;
  margin-left: auto;
  margin-right: auto;
  padding-top: 2rem;
  padding-bottom: 2rem;

  &.v-demo__section--no-padding {
    padding-top: 0;
    padding-bottom: 0;
  }

  &.v-demo__section--no-margin {
    padding-left: 0;
    padding-right: 0;
  }

  &.v-demo__section--intro {
    box-sizing: content-box;
    position: relative;

    p:nth-child(1n + 1) {
      color: var(--app-color-main);
    }

    p:nth-child(2n + 1) {
      color: var(--app-color-main--dark);
    }
  }

  .v-demo__section__graphic-items {
    position: absolute;

    @media (max-width: 900px) {
      display: none;
    }
  }

  .v-demo__section__graphic-items--m {
    top: 0;
    left: 0;
    transform: translate(-100%, 0%);
    height: 16vw;
    max-height: 298px;
  }

  .v-demo__section__graphic-items--o {
    bottom: 0;
    left: 0;
    transform: translate(-100%, 0%);
    height: 8vw;
    max-height: 148px;
  }

  .v-demo__section__graphic-items--du {
    top: 0;
    right: 0;
    transform: translate(90%, -10%);
    height: 20vw;
    max-height: 372px;
  }

  .v-demo__section__graphic-items--s {
    bottom: 0;
    right: 0;
    transform: translate(100%, 0%);
    height: 12vw;
    max-height: 224px;
  }
}

.v-demo__section__video-box {
  display: grid;
  width: 100%;
  grid-template-columns: repeat(2, .5fr);
  gap: var(--app-gutter);
  padding-top: 2rem;
}

.v-demo__section__video-box__player {
  width: 100%;
  max-width: 20rem;
  margin: auto;

  &.v-demo__section__video-box__player--xl {
    grid-column-end: span 2;
    max-width: 28rem;
  }

  @media (max-width: 1200px) {
    grid-column-end: span 2;
  }
}

.v-demo__section__content {
  position: relative;

  > img {
    display: block;
    width: 100%;
  }

  &.v-demo__section__content--sticky {
    @media (max-width: 700px) {
      flex-direction: column;
    }

    > * {
      @media (max-width: 900px) {
        width: calc(((100% + var(--app-gutter)) / 5 * 3) - var(--app-gutter)) !important;
      }
      @media (max-width: 700px) {
        width: calc(((100% + var(--app-gutter)) / 1) - var(--app-gutter)) !important;
      }
    }

    .v-demo__section__content--sticky__coll {
      @media (max-width: 900px) {
        width: calc(((100% + var(--app-gutter)) / 5 * 2) - var(--app-gutter)) !important;
      }
      @media (max-width: 700px) {
        width: 100% !important;
      }
    }

    .v-demo__section__content--sticky__coll__text-sticky {
      position: sticky;
      top: calc(var(--app-nav__height) * 2);
    }

    .v-demo__section__content--sticky__coll__element-sticky {
      position: sticky;
      top: var(--app-nav__height);

      > img {
        display: block;
        width: calc(100% + var(--app-gutter));
        height: auto;
        object-fit: cover;
        object-position: bottom;

        @media (max-width: 900px) {
          width: calc(100vw / 5 * 2);
        }

        @media (max-width: 700px) {
          margin-left: calc(-1 * var(--app-gutter));
          margin-right: calc(-1 * var(--app-gutter));
          height: 75vh;
          margin-bottom: -2rem;
          width: calc(100% + var(--app-gutter) * 2);
        }
      }
    }
  }

  &.v-demo__section__content--list-with-img {
    display: flex;
    flex-wrap: wrap;
    justify-content: center;
    row-gap: 2rem;
    column-gap: 2rem;
    margin-top: 2rem;

    .v-demo__section__content--list-with-img__item {
      box-sizing: border-box;
      width: calc(50% - 1rem);
      align-items: flex-start;
      display: flex;
      flex-direction: column;

      @media (max-width: 900px) {
        width: 100%;
        align-items: center;
      }
    }

    .v-demo__section__content--list-with-img__item__img {
      display: block;
      width: 4rem;
      height: 4rem;
      flex-shrink: 0;
      padding-bottom: .5rem;
      padding-left: .5rem;

      @media (max-width: 900px) {
        width: 5rem;
        height: 5rem;
        padding-bottom: 0;
        padding-left: 0;
      }
    }

    .v-demo__section__content--list-with-img__item__text {
      width: 100%;
      flex-shrink: 1;
      box-sizing: border-box;
      padding-left: .5rem;
      font-size: .55rem;
      line-height: 1.25em;
      @media (max-width: 900px) {
        margin-top: 1rem;
      }
    }

    .v-demo__section__content--list-with-img__item__text__title {
      margin-bottom: .5rem;
      font-size: .65rem;
    }
    .v-demo__section__content--list-with-img__item__text__content {
      color: var(--app-color-main);
    }
  }
}


.v-demo__section__title {
  display: flex;
  flex-direction: column;
  align-items: center;

  > * {
    margin-top: 2rem;
    margin-bottom: 2rem;
  }

  h3 {
    text-align: center;
    margin-bottom: 0;
    line-height: 1.25em;

  }
  h2 + h3 {
    margin-top: 0;
  }
}

.v-demo__grid {
  display: flex;
  flex-wrap: nowrap;

  &.v-demo__grid--with-gutter {
    gap: var(--app-gutter);
  }

  &.v-demo__grid--wrap {
    flex-wrap: wrap;
  }

  &.v-demo__grid--centred {
    justify-content: center;
  }
}

[class*="v-demo__grid__coll"] {
  > *:first-child {
    margin-top: 0 !important;
  }

  > *:last-child {
    margin-top: 0 !important;
  }
}

.v-demo__grid__coll-full {
  width: 100%;
}

.v-demo__grid__coll-1-2 {
  width: 50%;

  .v-demo__grid--with-gutter & {
    width: calc(((100% + var(--app-gutter)) / 2) - var(--app-gutter));
  }

  @media (max-width: 900px) {
    width: 75%;

    .v-demo__grid--with-gutter & {
      width: calc(((100% + var(--app-gutter)) / 3) - var(--app-gutter));
    }
  }

  @media (max-width: 700px) {
    width: 100%;

    .v-demo__grid--with-gutter & {
      width: 100%;
    }
  }
}

.v-demo__grid__coll-1-4 {
  width: 25%;

  .v-demo__grid--with-gutter & {
    width: calc(((100% + var(--app-gutter)) / 4) - var(--app-gutter));
  }

  @media (max-width: 1200px) {
    width: calc(100% / 3);

    .v-demo__grid--with-gutter & {
      width: calc(((100% + var(--app-gutter)) / 3) - var(--app-gutter));
    }
  }

  @media (max-width: 900px) {
    width: 50%;

    .v-demo__grid--with-gutter & {
      width: calc(((100% + var(--app-gutter)) / 2) - var(--app-gutter));
    }
  }

  @media (max-width: 700px) {
    width: 100%;

    .v-demo__grid--with-gutter & {
      width: 100%;
    }
  }
}

.v-demo__grid__coll-1-3 {
  width: calc(100% / 3);

  .v-demo__grid--with-gutter & {
    width: calc(((100% + var(--app-gutter)) / 3) - var(--app-gutter));
  }

  @media (max-width: 900px) {
    width: 50%;

    .v-demo__grid--with-gutter & {
      width: calc(((100% + var(--app-gutter)) / 2) - var(--app-gutter));
    }
  }


  @media (max-width: 700px) {
    width: 100%;

    .v-demo__grid--with-gutter & {
      width: 100%;
    }
  }
}

.v-demo__grid__coll-3-4 {
  width: 75%;

  .v-demo__grid--with-gutter & {
    width: calc(((100% + var(--app-gutter)) / 4 * 3) - var(--app-gutter));
  }

  @media (max-width: 1200px) {
    width: 100%;

    .v-demo__grid--with-gutter & {
      width: 100%;
    }
  }
}

.v-demo__content {
  background: var(--app-color-grey);
  position: relative;
  z-index: 10;
  width: 100%;

  @media (max-width: 900px) {
    > section:first-child {
      padding-top: 0;
    }
  }
}

.app-schema_animation {
  > img {
    display: block;
    height: auto;
    position: absolute;
    z-index: 1;


    &:not(.not-animated) {
      transform-origin: center center;
      transition: opacity 0s cubic-bezier(.25, 0, .75, 0), transform 0s cubic-bezier(0, 0, 1, 1);
      transition-duration: .25s;
      transition-delay: .25s;
      opacity: 0;
      transform: translate(0, 10%) scale(.75) rotate(5deg);
    }

    &.is-circle {
      transition: opacity 0s cubic-bezier(.25, 0, .75, 0), transform 0s cubic-bezier(0, 0, 1, 1);
      transition-duration: .25s;
      opacity: 0;
      transform: translate(0, 0) scale(.85) rotate(0);
      transition-delay: .5s !important;
    }
  }

  &.is-active > img {
    opacity: 1;

    &:not(.not-animated) {
      transition-duration: .75s;

      $increment: 0.05s;
      @for $i from 1 through 20 {
        &:nth-child(#{$i}n) {
          transition-delay: #{($i - 1) * $increment};
        }
      }
    }

    &.is-circle {
      transition-duration: .5s;
    }

    transform: translate(0, 0) scale(1) rotate(0);
  }
}

.app-schema_1 {
  max-width: 25rem;
  margin: auto;

  > img {
    display: block;
    height: auto;
    position: absolute;
    z-index: 1;
  }

  .app-schema_1_0 {
    width: 100%;
    position: relative;
    opacity: 0 !important;
    pointer-events: none;
    z-index: -1;
  }

  .app-schema_1_1 {
    top: 92.4%;
    left: 75.1%;
    width: 22.1%;
  }

  .app-schema_1_2 {
    top: 8.3%;
    left: 3.9%;
    width: 8.7%;
  }

  .app-schema_1_3 {
    top: 1.3%;
    left: 57.5%;
    width: 23.3%;
  }

  .app-schema_1_4 {
    top: 33.3%;
    left: 74.5%;
    width: 25.4%;
  }

  .app-schema_1_5 {
    top: 54%;
    left: 62.5%;
    width: 23%;
  }

  .app-schema_1_6 {
    top: 65.3%;
    left: 34.5%;
    width: 25.8%;
  }

  .app-schema_1_7 {
    top: 62.4%;
    left: 0;
    width: 28.3%;
  }

  .app-schema_1_8 {
    top: 21.4%;
    left: 1%;
    width: 23%;
  }

  .app-schema_1_9 {
    top: 0;
    left: 23%;
    width: 23%;
  }

  .app-schema_1_10 {
    z-index: 0;
    top: 8%;
    left: 16%;
    width: 61.7%;
  }

  .app-schema_1_11 {
    z-index: 0;
    top: 41.8%;
    left: 29.5%;
    width: 35%;
  }
}


.app-schema_2 {
  max-width: 25rem;
  margin: auto;
  margin-top: 2rem;

  > img {
    display: block;
    height: auto;
    position: absolute;
    z-index: 1;
  }

  .app-schema_2_0 {
    width: 100%;
    position: relative;
    opacity: 0 !important;
    pointer-events: none;
    z-index: -1;
  }

  .app-schema_2_1 {
    top: 67.5%;
    left: 76.5%;
    width: 23.5%;
  }

  .app-schema_2_2 {
    top: 66.8%;
    left: 24.2%;
    width: 25%;
  }

  .app-schema_2_3 {
    top: 14.8%;
    left: 0.1%;
    width: 35%;
  }

  .app-schema_2_4 {
    top: 0.8%;
    left: 67.5%;
    width: 29.8%;
  }

  .app-schema_2_5 {
    top: 45.4%;
    left: 37.4%;
    width: 36.7%;
  }

  .app-schema_2_6 {
    z-index: 0;
    top: 9.8%;
    left: 23.5%;
    width: 64.4%;
  }

}

</style>
